...
# %% auto 0
__all__ = ['ShiftAndLogScale', 'AffineCoupling_v1', 'AffineCoupling', 'ConditionalAffineCoupling', 'ConditionalAffine']
...
class ShiftAndLogScale(nn.Module):
    def __init__(self, num_in, num_out, width=4, ndim=2, shift_only=False, activation=nn.ReLU, device='cpu'):
        ...
        self.conv_1 = ConvLayer(self.num_in, self.width, ndim=self.ndim, ks=3, act_cls=activation, padding=1, init=nn.init.normal_) #
        self.conv_2 = ConvLayer(self.width, self.width, ndim=self.ndim, ks=1, act_cls=activation, padding=0, init=nn.init.normal_)
        self.net = nn.Sequential(self.conv_1, self.conv_2)

        padding = (1,) * (2 + self.ndim) + (0, 1)
        self.padding = nn.ConstantPad3d(padding, 0.)    # self.padding = nn.ConstantPad3d((1, 1, 1, 1, 0, 1), 0.)
        
        self.conv_3 = ConvLayer(self.width + 1, self.num_out, ndim=self.ndim, ks=3, act_cls=None, padding=0, init=nn.init.zeros_)
        self.logs = nn.Parameter(torch.zeros([1, self.num_out, 1, 1], device=device))
...
class AffineCoupling_v1(nn.Module):
    def __init__(self, x_shape, shift_and_log_scale, name="real_nvp", device='cuda'):
        super(AffineCoupling_v1, self).__init__()
        self.x_shape = x_shape
        self.ic, self.i0, self.i1 = x_shape
        self._shift_and_log_scale = shift_and_log_scale(num_in=self.ic // 2, num_out=2*(self.ic  - self.ic // 2), device=device, ndim=len(x_shape))
        self.name = name
...
# %% ../../nbs/02_layers_affine_coupling.ipynb 9
class AffineCoupling(nn.Module):
    def __init__(self, in_channels, hidden_channels, activation=nn.ReLU()):
...
# %% ../../nbs/02_layers_affine_coupling.ipynb 12
...
# %% ../../nbs/02_layers_affine_coupling.ipynb 13

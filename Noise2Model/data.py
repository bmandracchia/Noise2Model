...
# %% ../nbs/01_data.ipynb 8
def get_png_img_paths(base_path):
    file_paths = []
    for root, dirs, files in os.walk(base_path):
        if 'raw' in dirs:
            gt_folder = 'gt' if 'gt' in dirs else ('GT' if 'GT' in dirs else None)
            if gt_folder:
                raw_dir = os.path.join(root, 'raw')
                gt_dir = os.path.join(root, gt_folder)
...
# %% ../nbs/01_data.ipynb 9
def get_tif_img_paths(base_path):
    ...
    for root, dirs, files in os.walk(base_path):
        ...
        for f in tif_files:
            if f.endswith('lowsnr.tif'):
                base = f[:-len('lowsnr.tif')]
                groups[base].append(('lowsnr', os.path.join(root, f)))
            elif f.endswith('highsnr.tif'):
                base = f[:-len('highsnr.tif')]
                groups[base].append(('highsnr', os.path.join(root, f)))
            ...
            if len(items) == 2:
                ...
                for typ, path in items:
                    if typ == 'lowsnr':
                        ...
                        gt = path
                if noisy and gt:
                    file_paths.append({'GT': gt, 'NOISY': noisy})
    return file_paths

# %% ../nbs/01_data.ipynb 12
def convert_setup_code(model_name):
    models = ['100x', '060x', '040x', '020x']
...
def parse_file_name(path):
    file_name = path['GT'].split('/')[-1].split('.')[0]  # Remove file extension
    tokens = file_name.split('-')
    return {
        'scene-instance-number': int(tokens[-1]),
        'scene-number': int('001'),
        'optical-setup': int(convert_setup_code('020x')),
        'camera': int(convert_camera_code('sCMOS')), 
        'exposure-time': int('00060'), # ms
        'wavelength': int('600'), # nm
        'sample-code':int(convert_sample_code('nothing')) 
    }
